...
public class ChordLookup {
    ...
    public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
        ...
        boolean inRage = Util.checkInterval(key, nodID, succID);
    ...
    private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
        ...
        for (int i = table.size() - 1; i >= 0; i--) {
            ...
            String name = finger.getNodeName();
            int port = finger.getPort();

            NodeInterface stub = Util.getProcessStub(name, port);

            BigInteger fingerID = stub.getNodeID();

            boolean inRange = Util.checkInterval(fingerID, nodeID.add(BigInteger.ONE), ID);
